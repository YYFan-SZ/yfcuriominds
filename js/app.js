...
function handleTagFilter(clickedTag) {
    ...
    // 新增：针对英文标签增加同义词映射，确保过滤命中
    const synonyms = {
        en: {
            numbers: ['numbers', 'math', 'number'],
            memory: ['memory', 'memorize', 'remember'],
            strategy: ['strategy', 'strategic'],
            logic: ['logic'],
        },
        zh: {
            // 允许英文data-tag映射中文分类/标签
            numbers: ['数字', '数学'],
            logic: ['逻辑'],
            memory: ['记忆'],
            strategy: ['策略'],
    ...
    // 根据当前语言获取对应的同义词数组（若无则使用自身）
    const synonymList = (synonyms[currentLang] && synonyms[currentLang][tagValue]) ? synonyms[currentLang][tagValue] : [tagValue];
    ...
    if (tagValue === 'all') {
        ...
        filteredGames = games.filter(game => {
            ...
            // 检查分类或标签是否匹配任一同义词
            const categoryMatch = synonymList.some(syn => category.toLowerCase().includes(syn.toLowerCase()));
            const tagMatch = tags.some(tag => synonymList.some(syn => tag.toLowerCase().includes(syn.toLowerCase())));
...
function handleCategoryFilter(category) {
    if (category === 'all') {
        ...
        filteredGames = games.filter(game => {
            // 统一使用英文分类名进行匹配
            return game.category_en === category;
        });
    }
